...
# DATABASE CONFIGURATION (Resilient for Local vs Production)
...
# Logic to determine the final URI
if not db_url and IS_VERCEL:
    # On Vercel, we cannot write to SQLite. Use in-memory to prevent boot crash.
...
def initialize_database():
    """Ensure DB is ready. If on Vercel and no DB_URL is found, show instructions."""
    if IS_VERCEL and not db_url:
        return "<h1>Arua City System: Setup Required</h1><p>Vercel detected. Please add your <b>DATABASE_URL</b> to the Vercel Environment Variables to go live.</p>", 500
    ...
    if not hasattr(app, '_db_initialized'):
        try:
            ...
            return f"<h1>Database Connection Error</h1><p>The system could not connect to your Supabase database. Error: {str(e)}</p>", 500
...
@app.route('/')
def index():
    """Redirect home page to the dashboard."""
...
if __name__ == '__main__':
    ...
    # We do a quick check to see if the host is reachable before initializing the engine
    ...
    # 2. Fallback to Local SQLite if primary failed or isn't set
    if not connection_successful:
        # Re-configure to use local SQLite
        app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + os.path.join(app.instance_path, 'vendors_v2.db')
        # Re-initialize the engine with the new URI
        with app.app_context():
            db.create_all()
            seed_demo_data()
        print("SUCCESS: Local database (SQLite) is active.")

    # 3. Start the server
    print("\nREADY: Arua City System is running at http://127.0.0.1:5000")
    app.run(debug=True, use_reloader=False) # Reloader disabled to prevent double-init in dev
